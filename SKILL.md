...
*   **Early PR Lint & Typecheck Gates**: You MUST configure Pull Request CI gates to run lightweight static analysis (specifically `npm run lint` and `npm run typecheck` for the frontend, and `ruff check` and typechecking for the backend) as early as possible. Running these gates immediately on every PR creation/update ensures that syntax, lint, or compilation issues are flagged before resource-heavy integration or E2E tests are scheduled to run, optimizing both developer feedback loops and CI runner consumption.
