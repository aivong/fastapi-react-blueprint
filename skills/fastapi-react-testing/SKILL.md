...
* **Backend**: `pytest` for domain logic. Follow TDD (Red-Green-Refactor cycles; load the optional [tdd](../tdd/SKILL.md) skill if available). Run `mutmut` against domain modules to prove test effectiveness.
* **Frontend**: Vitest Browser Mode (preferred) or DOM Testing Library for behavior-driven UI tests (load the optional [front-end-testing](../front-end-testing/SKILL.md) skill if available).
